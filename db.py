...
def get_top_tips():
	...
	if user24h is not None:
		result += "Biggest tip in the last 24 hours:```%.2f BANANO by %s```" % (amount24h, user24h)
	if monthuser is not None:
		result += "Biggest tip in %s:```%.2f BANANO by %s```" % (month_str, monthamount, monthuser)
	if atuser is not None:
		result += "Biggest tip of all time:```%.2f BANANO by %s```" % (atamount, atuser)
...
# Update tip amount for stats (this value is saved as BANANO not xrb)
